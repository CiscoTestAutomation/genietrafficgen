...
Output DB/Excel Files Specifications
--------------------------

The ``export_results_as_db`` method produces two types of files: an SQLite database and an Excel report.

Database File (.db)
^^^^^^^^^^^^^^^^^^^

The exported ``.db`` file is a direct copy of the results database generated by Spirent TestCenter.

* **Integrity**: The script saves the database file exactly as it is output by the Spirent TestCenter. No tables are modified, deleted, or filtered within the file itself.
* **Filename**: While the internal content remains identical to the original, the file is saved using the sanitized version of the ``db_filename`` argument (default: ``stc_results.db``).

Excel Report (.xlsx)
^^^^^^^^^^^^^^^^^^^^

The Excel report is generated by extracting and joining specific tables from the aforementioned ``.db`` file. It provides a "Advanced Sequencing" view by mapping database values directly to spreadsheet columns.

* **Filename**: The output filename is sanitized from ``xlsx_filename`` (default: ``advanced_sequencing_all.xlsx``).

**Table Join Logic**

To construct the report, the script performs a ``JOIN`` on the following tables from the database:

* ``RxEotStreamResults``
* ``TxEotStreamResults``
* ``StreamBlock``

The tables are joined using ``DataSetId`` and ``StreamIndex`` as primary keys to ensure data alignment.

**Data Mapping**

Most columns in the Excel report represent the raw values stored in the ``.db`` file. Only a few columns (such as loss percentage) are calculated during the export process.
...
.. list-table::
   ...
   * - **Tx-Rx (Frames)**
     - N/A
     - Difference between ``TxEotStreamResults`` table's column ``FrameCount`` and ``RxEotStreamResults`` table's column ``FrameCount`` when Tx ``FrameCount`` > 0
   * - **Tx-Rx (%)**
     - N/A
     - Percentage of the difference between ``TxEotStreamResults`` table's column ``FrameCount`` and ``RxEotStreamResults`` table's column ``FrameCount`` relative to ``TxEotStreamResults`` table's column ``FrameCount`` when Tx ``FrameCount`` > 0
   * - **Dropped Frame Count**
     - ``RxEotStreamResults``
     - ``DroppedFrameCount``
   * - **Dropped Frame (%)**
     - N/A
     - Percentage of ``RxEotStreamResults`` table's column ``DroppedFrameCount`` relative to ``TxEotStreamResults`` table's column ``FrameCount`` when Tx ``FrameCount`` > 0
   * - **In-order Count (Frames)**
     - ``RxEotStreamResults``
     - ``InOrderFrameCount``
   * - **Reordered Count (Frames)**
     - ``RxEotStreamResults``
     - ``ReorderedFrameCount``
   * - **Duplicate Count (Frames)**
     - ``RxEotStreamResults``
     - ``DuplicateFrameCount``
   * - **Late Count (Frames)**
     - ``RxEotStreamResults``
...
The resulting dataset is sorted to ensure chronological and logical consistency across multiple test runs:

* Results are first grouped by **DataSetId** (ascending).
* Within each DataSet, rows are ordered by **StreamIndex** (ascending).
...
.. code-block:: text
    ...
    |                                 |           Default: 10 (seconds)                |
    |---------------------------------+------------------------------------------------|
    | save_results_as_db              | Saves results internally on Spirent API server |
    |                                 | without downloading a local file.              |
    |                                 | Arguments:                                     |
    |                                 |     * [O] clear_statistics - if True, clears   |
    |                                 |           statistics after save.               |
    |                                 |           Default: True                        |
    |                                 | Returns:                                       |
    |                                 |     * True - if save succeeds.                 |
    |---------------------------------+------------------------------------------------|
    | export_results_as_db            | Exports results from Spirent as a database     |
    |                                 | file and generates an Excel report.            |
    |                                 | Arguments:                                     |
    |                                 |     * [O] db_filename - Filename for the       |
    |                                 |           exported database file.              |
    |                                 |           Default: 'stc_results.db'            |
    |                                 |     * [O] xlsx_filename - Filename for the     |
    |                                 |           Excel report. Set to ``None`` to     |
    |                                 |           skip Excel generation.               |
    |                                 |           Default: 'advanced_sequencing_all.xlsx'|
    |                                 | Note: both ``db_filename`` and ``xlsx_filename``|
    |                                 | are sanitized before saving locally.           |
    |                                 | Returns:                                       |
...
.. code-block:: python
    ...
    # Save results on Spirent API server and clear stats for next DataSet
    >> dev.save_results_as_db(clear_statistics=True)
    True
    # Export Spirent results as local database file
    >> dev.export_results_as_db(db_filename='run1_results.db')
    True
    # Export both DB and Excel using explicit filenames
    >> dev.export_results_as_db(db_filename='run2_results.db', xlsx_filename='run2_advanced.xlsx')
    True
