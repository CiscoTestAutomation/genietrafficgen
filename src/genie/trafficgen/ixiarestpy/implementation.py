...
class IxiaRestPy(TrafficGen):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.clear_config_timeout = connection_args.get('clear_config_timeout', 180)
    ...
    def _new_config_with_retry(self):
        timeout = Timeout(max_time=self.clear_config_timeout, interval=30, disable_log=False)
        last_error = None
        attempt = 0
        while timeout.iterate():
            try:
                ...
                if 'loading configuration' in str(e).lower():
                    attempt += 1
                    last_error = e
                    logger.warning(
                        f'IxNetwork Loading Configuration in progress, retrying... (attempt {attempt})'
                    )
                    timeout.sleep()
                else:
                    raise
        raise GenieTgnError(
            f'IxNetwork still loading configuration after {self.clear_config_timeout}s'
        ) from last_error
